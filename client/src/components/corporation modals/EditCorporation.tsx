...
export const EditCorporationSchema = Yup.object({
...
const EditCorporation = NiceModal.create((props: Props) => {
  ...
  return (
    ...
    >
      <DialogTitle>
        <Typography textAlign="center">Edit corporation info</Typography>
      </DialogTitle>
      <Formik
        initialValues={{ name: name, description: description }}
        validationSchema={EditCorporationSchema}
        onSubmit={(values) => {
          try {
            corporationApi.updateCorporation(values, id).then(() => {
              confirm();
              modal.remove();
            });
          } catch (error) {
